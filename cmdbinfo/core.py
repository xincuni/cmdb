...
class Asset(object):
    ...
    def mandatory_check(self,data,only_check_sn=False):
        ...
        for field in self.mandatory_fields:
            #for i in [sn,asset_id,asscet_type]
        ...
        try:
            ...
            if not only_check_sn:
                #通过id 和sn 从数据库当中过滤得到id=asset_id的对象
                #asset_obj = 7.querset
    ...
    def data_is_valid(self):
        # ?asset_data={'asdasdasd'}
        data = self.request.POST.get("asset_data")
        if data:
            try:
                #json转换成字典 data就是一个字典 字典里面包含了我们所有的数据
                data = json.loads(data)
                self.mandatory_check(data)
                #clean_data就是我们前端传过来的数据
                self.clean_data = data
    ...
    def __is_new_asset(self):
        # asset_type = server network scrity  负负得正的关系
        #self.asset_obj它是一个完整的queryset资产对象   clean_data['asset_type']= 'server ' asset_type
        #hasattr(self.asset_obj, self.clean_data['asset_type'])  server存在  返回true    false
        if not hasattr(self.asset_obj, self.clean_data['asset_type']):#new asset server
            return True
        else:
            return False
    ...
    def data_is_valid_without_id(self):
        ...
        if data:
            try:
                ...
                #data['asset_id'] = asset_obj[0].id
                self.mandatory_check(data)
                #accets.obj(queryset 包含了我们这个资产里面所有的信息)
                self.clean_data = data
                #asset_id
                if not self.response['error']:
                    return True
    ...
    def create_asset(self):
        ...
        #getattr(self,'_create_server')
        create_obj =func()

    def update_asset(self):
        #self.clean_data['asset_type'] = server
        # func = _update_server
        func = getattr(self,'_update_%s' % self.clean_data['asset_type'])
        create_obj =func()

    def _update_server(self):
        # data_source = agent传过来的数据[nic ] 网卡
        #fk = 'nic_set' 通过资产总表 查看反向外键关联的表
        # update_fields 需要更新的字段 校验的字段
        # identify_filed 判断依据
    ...
    def __update_asset_component(self,data_source,fk,update_fields,identify_field=None):
        ...
        try:
            #asset_obj是一个queryset对象  nic_set
            #以网卡为例   component_obj包含了我们所有的网卡queryset对象
            component_obj = getattr(self.asset_obj,fk)
            print '------------------------------->'
            # print  component_obj = self.asset_obj.nic_set
            if hasattr(component_obj,'select_related'): # this component is reverse m2m relation with Asset model
                #select_related()会优化我们数据库的执行数据
                #objects_from_db 所有的网卡信息 数据库里面拿过来的
                # component_obj 客户端传过来的
                objects_from_db = component_obj.select_related()
                for obj in objects_from_db:
                    #obj就是一个网卡 identify_field=macaddress getattr去对比  如果有有macadress就获取macadress
                    ...
                    if type(data_source) is list:
                        ...
                        for key,source_data_item  in data_source.items():
                            print('-------------disk----dicst')
                            #获取从客户端传过来的数据的mac地址   identify_field=macadress key_field_data_from_source_data客户端的mac地址
                            key_field_data_from_source_data = source_data_item.get(identify_field)
                            if key_field_data_from_source_data:
                                #key_field_data 从数据库里面取出来的网卡的mac地址  key_field_data_from_source_data=客户端的mac地址
                                if key_field_data == key_field_data_from_source_data: #find the matched source data for this component,then should compare each field in this component to see if there's any changes since last update
                                   self.__compare_componet(model_obj=obj,fields_from_db=update_fields,data_source=source_data_item)
                                   break #must break ast last ,then if the loop is finished , logic will goes for ..else part,then you will know that no source data is matched for by using this key_field_data, that means , this item is lacked from source data, it makes sense when the hardware info got changed. e.g: one of the RAM is broken, sb takes it away,then this data will not be reported in reporting data
                            else: #key field data from source data cannot be none
                ...
                #model_obj_name 表的名称   data_from_db==objects_from_db
    ...
    def __filter_add_or_deleted_components(self,model_obj_name,data_from_db,data_source,identify_field):
        #model_obj_name 表的名称   data_from_db,data_source,identify_field
        ...
        #data_source_key_list 里面包含了我们客户端传来网卡信息 mac地址
        ...
        #python 集合(set)默认是去重
        #for obj in data_from_db  每一个列表都是一个mac
        #得到2个集合  1个集合包含客户端传过来的mac  一个集合包含数据库里面这个资产的mac地址
        data_identify_val_from_db = set([getattr(obj,identify_field) for obj in data_from_db])
        #第一种情况 数据库里面的数据比客户端传过来的数据要多  data_only_in_db=得到数据库当中的值
        #第二种情况 数据库里面的数据和客户端里面的数据一模一样  data_only_in_db=空
        #第三种情况 数据库里面的数据比客户端里面的数据要少      data_only_in_db=空
        #如果客户端的数据比数据库里面的数据要多 我是不要添加网卡
        #data_only_in_db 只有数据库里面的数据比客户端传过来的数据多的时候他才有数据 删除这个数据
        #data_only_in_data_source 只有客户端传过来的数据比较多的时候才有数据 把这个数据添加到数据库里面
    ...
    def __add_components(self,model_obj_name,all_components,add_list,identify_field ):
        ...
        try:
            for component in will_be_creating_list:
                data_set = {}
                #model_call =nic。auto_create_fields
                #model_class.auto_create_fields= ['name','sn','model','macaddress','ipaddress','netmask','bonding']
                for field in model_class.auto_create_fields:
                    data_set[field] = component.get(field)

                data_set['asset_id'] = self.asset_obj.id
                # del data_set['manufactory']
    ...
    def __delete_components(self,all_components, delete_list , identify_field ):
        ...
        #all_components 数据库当中所有的数据 delete_list 需要删除的数据 identify_field 关键字
        deleting_obj_list = []
        ...
        #deleting_obj_list需要删除的queryset对象(网卡)
    ...
    def __compare_componet(self,model_obj,fields_from_db,data_source):
        #model_obj 表的对象，fileds_from_db 这个就是网卡的queryset对象，data_source=客户端传过来的数据[NIC]
        ...
        for field in fields_from_db:
            ...
            #val_from_db得到数据库里面有个网卡的queryset的对象   val_from_data_source 网卡的对象
            #这个判断可以不用
            if val_from_data_source:
                ...
                #从数据库里面获取的数据肯定是正确的  但是我们从客户端传过来的数据有可能不正确
