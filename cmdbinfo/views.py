...
import json
from cmdbinfo import core, models, asset_handle, utils, admin
from django.contrib.auth.decorators import login_required
from django.views.decorators.csrf import csrf_exempt
from django.core.exceptions import ObjectDoesNotExist
from cmdbinfo import tables
from django.core.paginator import Paginator, EmptyPage, PageNotAnInteger
# from cmdbinfo import models
# from cmdbinfo import core
import utils
from cmdbinfo.dashboard import AssetDashboard
...
def asset_report(request):
    ...
    if request.method == 'POST':
        ...
        return HttpResponse(json.dumps(ass_handler.response))
        # return render(request,'cm01/asset_report_test.html',{'response':ass_handler.response})
        # else:
        # return HttpResponse(json.dumps(ass_handler.response))
...
def asset_with_no_asset_id(request):
    if request.method == 'POST':
        ...
        # return render(request,'cm01/acquire_asset_id_test.html',{'response':res})
...
def new_assets_approval(request):
    if request.method == 'POST':
        ...
        return render(request, 'cm01/new_cm01_approval.html',
                      {'new_cm01': approved_asset_list, 'response_dic': response_dic})
    else:
        ids = request.GET.get('ids')
        id_list = ids.split(',')
        new_cm01 = models.NewAssetApprovalZone.objects.filter(id__in=id_list)
        return render(request, 'cm01/new_cm01_approval.html', {'new_cm01': new_cm01})


def asset_report_test(request):
    return render(request, 'cm01/asset_report_test.html')


@login_required
def acquire_asset_id_test(request):
    return render(request, 'cm01/acquire_asset_id_test.html')
...
def asset_list(request):
    ...
    return render(request, 'cm01/cm01.html', {'table_obj': table_obj,
                                                  'paginator': paginator})
...
def asset_category(request):
    ...
    if request.is_ajax():
        ...
        return render(request, 'cm01/asset_category.html', {'category_type': category_type})
...
def asset_detail(request, asset_id):
    if request.method == "GET":
        try:
            ...
            return render(request, 'cm01/asset_detail.html', {'error': e})
        return render(request, 'cm01/asset_detail.html', {"asset_obj": asset_obj})
...
def get_dashboard_data(request):
    ...
    #data{'asset_categories':'{'names':'name','data'='1'}'}
